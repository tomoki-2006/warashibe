...
| T-U12 | ルート全体の結合（Title→Map→Encounter×7→Result・移動/マップ・結果カード） | 3L | 01 §1/§6/§10・04 §S2/S7準拠。Title→7ストップ完走→Resultまで通しで遊べる |

> T-U12 追記の経緯: U01〜U11 は Web版 E1〜E5 を置換したが、**マップ移動／ルート踏破（route walker, 01 §6）専用チケットが無く**、T-U09 の S0導線・舟イベントが単発デモでは発火しきらない。「今作っている単発デモを1本の旅として完成品にする」ため PO 合意で追加（2026-07-15）。オートセーブ／つづきから（01 §9）は分量の都合で本チケットからは分離（後続チケットで対応・現状セーブ無し＝つづきから非表示は §9 準拠）。
